...
public:
    ...
    double calculateGPA() const { return gpa; }
    void setGPA(double v) { if (v >= 0.0 && v <= 5.0) gpa = v; }
};
...
public:
    ...
    double calculateSalary() const { return salary + teachingCourses.size() * 5000.0; }
};
...
public:
    ...
    void setInstructor(Professor* prof) {
        instructor = prof;
        if (instructor) instructor->assignCourse(courseCode);
    }
...
public:
    ...
    void addStudent(unique_ptr<Student> s)   { members.push_back(move(s)); }
    void addProfessor(unique_ptr<Professor> p){ members.push_back(move(p)); }
